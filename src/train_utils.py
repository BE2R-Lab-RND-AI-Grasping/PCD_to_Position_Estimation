...
def train_model(model, train_loader, val_loader, optimizer, device, epochs=100, alpha=1.0, beta=1.0):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            gt_translation = batch[1][2].to(device)     # (B, 3)
            gt_quaternion = batch[1][1].to(device)       # (B, 4)
            # point_clouds = batch['points'].to(device)            # (B, N, 3)
            # class_labels = batch['class_label'].to(device)       # (B,)
            # gt_translation = batch['translation'].to(device)     # (B, 3)
            # gt_quaternion = batch['quaternion'].to(device)       # (B, 4)
        ...
        val_loss, val_cls_loss, val_pose_loss, val_accuracy = validate_model(model, val_loader, device)
